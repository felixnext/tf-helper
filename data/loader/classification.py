...
def _find_classes(folder, only):
  '''Retrieves the relevant classes from the dataset structure'''
  # generate data
...
def _gen_cls(folder, classes, shuffle=True, only=None, size=None, one_hot=True, beard_format=False, show_btype=False, resize=utils.ResizeMode.FIT, pad_color=(0, 0, 0), pad_mode=utils.PadMode.EDGE, debug=False):
    ...
    for btype in folders:
        ...
        for dir in folders[btype]:
            ...
            if not shuffle:
                for cls_name, gen in cls_gens:
                    for img in gen:
                        img, _, _ = lib.resize(
                            img, size, resize, pad_color, pad_mode)
        ...
        if not found:
            if debug: print(
                "Could not find folder for type: {}".format(btype.name))


def load(folder, imgSettings, classes=None, only=None, size=None, one_hot=True, beard_format=False, show_btype=False, debug=False, shuffle=True):
    '''Loads the classification data from file.

    Args:
      folder (str): folder where the dataset is stored
      imgSettings (ImageSettings): Settings to transform images
