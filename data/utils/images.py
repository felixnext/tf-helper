...
def get_padding_from_string(mode_str, resize_str, color=None):
  '''Retrieves padding values from string.
  ...
  if mode_str == "center":
    mode = PadMode.CENTER
  if resize_str == "stretch":
    resize = ResizeMode.STRETCH
  elif resize_str == "black":
    resize = ResizeMode.PAD_COLOR
    color = (0,0,0)
  elif resize_str == "blue":
    resize = ResizeMode.PAD_COLOR
    color = (0,0,255)
  elif resize_str == "red":
    resize = ResizeMode.PAD_COLOR
    color = (255,0,0)
  elif resize_str == "green":
    resize = ResizeMode.PAD_COLOR
    color = (0,255,0)
  elif resize_str == "color":
    resize = ResizeMode.PAD_COLOR
  elif resize_str == "random":
    resize = ResizeMode.PAD_RANDOM
  elif resize_str == "mean":
    resize = ResizeMode.PAD_MEAN
  elif resize_str == "edge":
    resize = ResizeMode.PAD_EDGE

  return mode, resize, color

def imread_resize(img_path, input_size, color_channels=3, resize=None, color=None, mode=None):
  ...
  img, _, _ = resize(img, input_size, resize, color, mode)
  return img
